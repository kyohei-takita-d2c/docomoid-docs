...
対象: `origin_apps/nidan-hera-js/src/nidan` 配下の TypeScript 全体。呼び出し契機ごとの概要シーケンスと関連ドキュメントの入口。
...
関連: Prebid 連携(pubProvidedId)時の ID 取得遅延の分析は [prebid_latency.md](prebid_latency.md) を参照。
